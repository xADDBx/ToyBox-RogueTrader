// The MIT License (MIT) Copyright(c) 2022 Tomáš Pažourek
...
public class NaturalSortComparer : IComparer<string> {
    ...
    /// Used if <see cref="m_StringComparer" /> is null.
    /// </summary>
    ...
    /// </summary>
    private readonly IComparer<string>? m_StringComparer;

    // Token values (not an enum as a performance micro-optimization)
    private const byte m_TokenNone = 0;
    private const byte m_TokenOther = 1;
    private const byte m_TokenDigits = 2;
    private const byte m_TokenLetters = 3;
    ...
    public int Compare(string str1, string str2) {
        ...
        while (true) {
            ...
            if (token1 == m_TokenDigits) {
                ...
                if (m_StringComparer is not null) {
                    ...
                    var stringCompare = m_StringComparer.Compare(tokenString1, tokenString2);
                    if (stringCompare != 0) {
                        return stringCompare;
                    }
                } else {
                    // use string comparison
                    var stringCompare = string.Compare(str1, startIndex1, str2, startIndex2, rangeLength1, m_StringComparison!.Value);
                    if (stringCompare != 0) {
                        return stringCompare;
                    }
                }
    ...
    private static byte GetTokenFromChar(char c) {
        return c switch {
            >= 'a' and <= 'z' => m_TokenLetters,
            >= 'a' when c < 128 => m_TokenOther,
            >= 'a' when char.IsLetter(c) => m_TokenLetters,
            >= 'a' when char.IsDigit(c) => m_TokenDigits,
            >= 'a' => m_TokenOther,
            >= 'A' and <= 'Z' => m_TokenLetters,
            >= 'A' => m_TokenOther,
            >= '0' and <= '9' => m_TokenDigits,
            _ => m_TokenOther,
        };
    }
}
